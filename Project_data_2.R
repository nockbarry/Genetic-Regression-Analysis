...
E <- regmatches(names(test), regexpr("(E[0-9])+", names(test)))
R <- regmatches(names(test), regexpr("(R[0-9]*)+", names(test)))
all = c(E,R)
form = paste(all,collapse = "+")
form
...
# or ?regsubsets
var_chose <- colnames(final_result$which)[final_result$which[10,]]
...
var_chose <- colnames(final_result$which)[final_result$which[4,]]
formula_select  <- paste0('I(log(Y)) ~ ', paste(var_chose[-1], collapse = '+') )
M <- lm(formula_select, data=test)
summary(M)
anova(M)
...
var_chose <- colnames(final_result$which)[final_result$which[5,]] # <- I use 10th here
